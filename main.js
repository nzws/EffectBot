...
function image_effect(imagetype, json, addtext = "") {
    ...
    loadImage('data/tmp/effect_user' + json["id"] + imagetype).then((image) => {
        ...
        if (mode["base"] === "vanila") {
            ...
            post_upimg("@" + json["account"]["acct"] + addtext + " " + mode["base"] + ":" + mode["type"] + " " + lang[json["language"] === "en" ? "en" : "ja"].lang[5], {}, config.post_privacy, false, 'data/tmp/effect_result' + json["id"] + imagetype);
                        ...
                        request({
                            ...
                            function (error, response, blob) {
                                if (!error && response.statusCode === 200) {
                                    ...
                                    post_upimg("@" + json["account"]["acct"] + addtext + " " + mode["base"] + ":" + mode["type"] + " " + lang[json["language"] === "en" ? "en" : "ja"].lang[5], {}, config.post_privacy, false, 'data/tmp/effect_result' + json["id"] + filet);
                                }
                            }
                        );
                    }
